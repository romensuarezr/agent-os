...
## Criterios de done
- [ ] Omitir la comprobación automáticamente si se detecta que el script se está ejecutando en el propio repositorio core.
- [ ] Implementar la lectura de `.agents/context/last-sync.md` y `.agents/context/agent-os-changelog.md`.
- [ ] Consultar el último commit del core remoto con `git ls-remote` aplicando un timeout estricto de 3 segundos y manejando fallos silenciosamente.
- [ ] Si hay diferencias de commits o la fecha en `last-sync.md` supera los 7 días de antigüedad:
...
- [ ] Validar que con la red desconectada el script continúe su flujo normal en menos de 4 segundos sin generar errores fatales.
- [ ] Compilación/Verificación sin errores.
